...
\author{ Jaahnavi Inala%<-this  stops a space
% <-this % stops a space
...
\\
\solution
\lstinputlisting[language=Python]{codes/1.py}
% \begin{figure}[h]
% \centering
...
\end{equation}
and $H(k)$ using $h(n)$.\\
% \solution Using \eqref{def:xn}:
% \begin{align}
%     X(k) &= \sum_{n=0}^{n=5}x(n)e^{-\j2\pi kn/6}\\
%     &= 1+2e^{-\j2\pi k/6}+3e^{-\j2\pi k/3}+4e^{-\j2\pi k/2}\nonumber\\
%     &+2e^{-\j2\pi 2k/3}+e^{-\j2\pi 5k/6}\\
%     &= 1+2e^{-\j\pi k/3}+3e^{-\j2\pi k/3}+4e^{-\j\pi k}\nonumber\\
%     &+2e^{-\j4\pi k/3}+e^{-\j5\pi k/3}
% \end{align}
% Similarly $H(k)$ can be calculated using \eqref{eq:hn}
% \begin{align}
%         H(k) &= \sum_{n=0}^{n=\N-1}h(n)e^{-\j2\pi kn/N}\\
